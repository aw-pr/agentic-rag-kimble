...
def test_running_task_with_passing_result_marks_done(tmp_path: Path) -> None:
    """A worker writes result.json with acceptance=pass; the next tick
    promotes the task to `done`."""
    exp = _exp_dir(tmp_path)
    state = tmp_path / "state.yaml"
    t = _task(
